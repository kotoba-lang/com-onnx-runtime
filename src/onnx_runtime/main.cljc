(ns onnx_runtime.main
  "Kotodama WASM entrypoint for the Onnx_runtime clean-room actor (L4) — Clojure port.
  ...
  facts (`onnx_runtime.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "onnx_runtime")
(def tier "L4")
...
(def entity-specs
  [{:entity "ModelProto"      :plural "modelprotos"      :id-prefix "onnxrunt_mod"
    :fields [:irVersion :producerName :producerVersion :domain :modelVersion :docString] :required [:irVersion :producerName]
    :coerce {:irVersion :int :modelVersion :int}     :refs {}}
   {:entity "GraphProto"      :plural "graphprotos"      :id-prefix "onnxrunt_gra"
    :fields [:name :docString]                         :required [:name :docString]
    :coerce {}                                         :refs {}}
   {:entity "NodeProto"       :plural "nodeprotos"       :id-prefix "onnxrunt_nod"
    :fields [:name :opType :domain :docString]         :required [:name :opType]
    :coerce {}                                         :refs {}}
   {:entity "TensorProto"     :plural "tensorprotos"     :id-prefix "onnxrunt_ten"
    :fields [:name :dataType :dataLocation :docString]  :required [:name :dataType]
    :coerce {:dataType :int :dataLocation :int}        :refs {}}
   {:entity "AttributeProto"  :plural "attributeprotos"  :id-prefix "onnxrunt_att"
    :fields [:name :type :docString]                   :required [:name :type]
    :coerce {:type :int}                               :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"onnx_runtime.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "onnx_runtime-compat" :tier tier :entities entities} 200])
...
(defn start! [] :onnx_runtime-compat/ready)
